...
public class DateTimePickDialogUtil implements OnDateChangedListener, OnTimeChangedListener {
	 ...
	 */
	Time_callback timeback;
	int res = 0;

	public DateTimePickDialogUtil(Activity activity, String initDateTime, int res, Time_callback timeback) {
		...
		ad = new AlertDialog.Builder(activity).setTitle(initDateTime).setView(dateTimeLayout)
				.setPositiveButton("确      定", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int whichButton) {
						timeback.dateTime(res, dateTime);
					}
				}).show();
	...
	public void onDateChanged(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
		...
		ad.setTitle(Utils.formatDate(dateTime, "yyyy年MM月dd日"));
	}
	...
	public interface Time_callback {
